...
## Para comprobar las relaciones de las tablas de mi app, corri los siguientes queries en la consola:
```ruby
User.all
User.where(name: "Dewayne Johnston CPA").update_all(name: "John Doe")
john_doe_posts = User.find_by(name: "John Doe")&.posts
```
## Esto para actualizar el nombre de uno de los usuarios que estoy creando (Dwayne Johnson ya existia). Asi que busque a todos los users, eleji cambiar el nombre a "John Doe" de uno de ellos y luego busque sus posts. Enuentra uno. 